...
const Row = ({ items, title, link }) => {
  ...
  return (
    <div>
      ...
      {title && (
        <h2 className="text-xl md:text-2xl font-semibold text-white ml-3">{title}</h2>
      )}
      <div className="flex">
      <Link className="mr-3 text-xs lg:text-sm ml-auto text-white/80 flex items-center capitalize" to={link}>View All<FaChevronRight /></Link>
      </div>
      </div>
      {loading ? (
...
function RowCard({ item, isMobile }) {
  const link = item.first_air_date? `/info/tv/${item.id}` : `/info/movie/${item.id}`;
